...
2. **校验脚本 `validate-social-deck.mjs`**。在 Step 5 渲染后自动跑：
   - 检查每张图的 `scrollHeight` 是否超过 board 高度（v0.6 用 Playwright `evaluate` 验证过这个方法）
   - 检查正文字号是否低于平台最小值
   - 检查 `.poster.xhs` 上的 `.h-xl` 是否超过硬上限
   把 "渲染前看不出来 / 渲染后才发现溢出" 这种坑用脚本兜底。
...
5. **地图组件**。旅行类内容经常需要"路线图 + 关键节点"。Mapbox 静态 API 出图能用，但目前要 AI 现写代码调用。可以做成一个 `.map-block` class，传 GeoJSON 进去自动出。
   **执行路径**：用 Mapbox Static Images API（免费额度够冒烟测试），出 1080×720 或 1080×1440 PNG，限制底图为 `light-v11` / `dark-v11` 两种以匹配 Editorial / Swiss 的中性基调，禁用花哨色板。节点用 Swiss 编号风（黑底白数字）或 Editorial 墨点风，避开默认蓝针。GeoJSON 仅接 LineString + Point，复杂面/标注超出范围。
