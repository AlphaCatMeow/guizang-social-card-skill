...
最后更新：2026-05-27 · 版本：v0.13（git 初始化 + 仓库瘦身：本地测试归集到 `local-tests/` 并 gitignore）
...
├── SKILL.md                   # 266 行 · 7 步工作流入口
├── HANDOFF.md                 # 本文件
├── PRODUCT.md                 # 产品思考
├── assets/
│   ├── template-editorial-card.html   # 585 行 · 杂志风种子
│   ├── template-swiss-card.html       # 740 行 · 瑞士风种子（v0.6 已硬化）
│   └── magazine-bg-webgl.js           # WebGL 墨流背景
├── references/                # 12 份子规范，按需读取
│   ├── platform-specs.md      (122) 平台 × 分辨率 × 命名
│   ├── style-system.md        (182) 两种风格的硬规则与反模式
│   ├── theme-presets.md       (212) 6 套杂志 palette（含 1 套暗色 Midnight Ink）+ 4 套 Swiss accent
│   ├── layout-recipes.md      (667) 27 个版式骨架（M01-M15 + S01-S12）
│   ├── components.md          (226) 字体 / 卡片 / 间距 / 图标 / 标题字数表
│   ├── background-systems.md  (108) 墨流 / 网格 / 纸纹层
│   ├── portrait-fill.md       (85)  3:4 板的留白对策
│   ├── content-planning.md    (138) 钩子 / 拆页 / 文案压缩
│   ├── production-workflow.md (172) 渲染管线与命令
│   ├── image-overlay.md       (150) 文字压图：蒙版 + 主体映射
│   ├── category-cookbook.md   (232) 小红书 11 个品类的版式路由
│   └── qa-checklist.md        (67)  交付前 checklist
├── agents/
...
文字压图的页（封面 / 大图位 / 全幅图）三步：先 `Read` 图、再写 HTML 注释记录主体位置、再选 `object-position`。蒙版是强制的。

---

## 6. 已知风险与限制（事实层面）

- **没有自动校验脚本**。所有"标题超过几个字会溢出"目前靠 components.md 的硬上限表 + 人工渲染 + 视觉判断。
